...
public class RN006 {
	...
	public Boolean isValidRN006(ActorDTO entidad) {
		...
		Actor actor;
		if (entidad.getId() == null) {
			actor = elementoDAO.findAllByIdProyectoAndNombreAndClave(Actor.class, entidad.getIdProyecto(), entidad.getNombre(), Clave.GLS);
		} else {
			actor = elementoDAO.findAllByIdProyectoAndIdAndNombreAndClave(Actor.class, entidad.getIdProyecto(), entidad.getId(), entidad.getNombre(), Clave.GLS);
		}
		if (actor != null) {
			valido = false;
		}
