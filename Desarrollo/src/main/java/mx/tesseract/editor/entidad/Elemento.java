...
@NamedNativeQueries({
	@NamedNativeQuery(name = "Elemento.findNextNumber", query = "SELECT COALESCE(MAX(CAST(e.numero AS SIGNED)), CAST('0' AS SIGNED)) FROM elemento e WHERE e.Proyectoid = ? AND e.clave = ?"),
	@NamedNativeQuery(name = "Elemento.findElementoHasAtributo", query = "select * from elemento e join reglanegocio r on r.Elementoid = e.id where e.Proyectoid = ? and r.Atributoid_comp1 = ? and r.Atributoid_comp2 = ? and r.Atributoid_expReg = ? and r.Atributoid_fechaI = ? and r.Atributoid_fechaT = ?"),
...
@NamedQueries({
	...
	@NamedQuery(name = "Elemento.consultarElementosByProyectoAndIdCasoUso", query = "SELECT e FROM Elemento e JOIN e.proyecto p WHERE p.id = :idProyecto"),
	...
	@NamedQuery(name = "Elemento.consultarPantallasByProyectoAndNumero", query = "SELECT e FROM Elemento e JOIN e.proyecto p JOIN e.modulo m WHERE p.id = :idProyecto AND e.clave = :clave AND e.numero = :numero"),
